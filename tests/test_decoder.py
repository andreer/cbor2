...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("replace", "hello�world"),  # Should replace invalid byte with �
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (b'\x79\x01\x05' + b'a' * 260 + b'\xFF', "replace", 'a' * 260 + '�'),  # >256: heap path
    ],
...
def test_str_errors_error_alias(impl):
    invalid_utf8 = b'\x6Bhello\xFFworld'
    with pytest.raises(impl.CBORDecodeValueError, match="error decoding unicode string"):
        impl.loads(invalid_utf8, str_errors="error")
